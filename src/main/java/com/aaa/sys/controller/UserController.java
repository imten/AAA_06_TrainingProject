...
public class UserController {
	...
	@ResponseBody
	public Boolean judgeUsername(@RequestBody JSONObject jsonObject) {
		String username=(String)jsonObject.get("username");
		QueryWrapper<User> queryWrapper=new QueryWrapper<>();
		queryWrapper.eq("loginname", username);
		List<User> user = userService.list(queryWrapper);
		if(user.size() ==0) return Constast.EXIST_FALSE;
		else {
			return Constast.EXIST_TRUE;}
	}
	...
	public DataGridView loadUserById(Integer id) {
		...
		queryWrapper.eq("del_flag", 0);
		List<User> list = this.userService.list(queryWrapper);
		if(list.size()!=0) return new DataGridView(list.get(0));
		else return null;
	...
	public ResultObj updateUser(UserVo userVo) {
		try {
			...
			systemLog("修改","修改用户（"+userVo.getName()+"-"+userVo.getLoginname()+")");
	...
	public ResultObj deleteUser(Integer id) {
		...
		try {
			...
			User us=list.get(0);
			us.setDel_flag(1);
			this.userService.updateById(us);
			systemLog("删除","删除用户（"+us.getName()+"-"+us.getLoginname()+"）");
